...
void print_exception(const hdf5::hdf5_error& e, int level =  0)
{
    std::cerr << std::string(level, ' ') << "exception: " << e.what() << "  " << translate_minor_error_code(e.minor_num()) << '\n';
    try
    {
        std::rethrow_if_nested(e);
    }
    catch(const hdf5::hdf5_error& e)
    {
        print_exception(e, level+1);
    }
    catch(...)
...
int main()
{
    ...
    {
        file my_file("/opt/test.hdf5", file::create_mode::truncate);
