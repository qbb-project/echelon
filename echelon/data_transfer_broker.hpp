...
inline typename std::enable_if<is_identity_lowering_function<
    type_lowering_hook<typename C::value_type>>::value>::type
write(Sink& sink, const hdf5::type& datatype, const hdf5::dataspace& memspace,
      const hdf5::dataspace& filespace, const C& container)
{
    sink.write(datatype, memspace, filespace, hdf5::default_property_list,
               data(container));
}

template <typename Sink, typename C>
inline typename std::enable_if<
    !is_identity_lowering_function<
         type_lowering_hook<typename C::value_type>>::value>::type
write(Sink& sink, const hdf5::type& datatype, const hdf5::dataspace& memspace,
      const hdf5::dataspace& filespace, const C& container)
{
     ...
     C& container)
{
    ...
    source.read(datatype, memspace, filespace, hdf5::default_property_list,
                data(container));
}
     ...
     C& container)
{
    ...
    typedef typename remove_base_type_cv<
        typename type_lowering_hook<T>::lowered_type>::type lowered_type;
        ...
        H5Tget_class(datatype.id()) == H5T_VLEN)
    {
        ...
        H5Dvlen_reclaim(datatype.id(), memspace.id(), H5P_DEFAULT,
                        lowered_data.data());
    }
}
...
template <typename Sink, typename T,
          typename std::enable_if<
              is_identity_lowering_function<type_lowering_hook<T>>::value,
              int>::type dummy = 0>
...
inline void read(Source& source, T& value)
{
        ...
        H5Tget_class(datatype.id()) == H5T_VLEN)
    {
        ...
        H5Dvlen_reclaim(datatype.id(), hdf5::dataspace().id(), H5P_DEFAULT,
                        &lowered_value);
    }
}
}
