...
public:
    ...
    {
        static_assert(std::is_convertible<Argument,OptionType>::value,"Invalid option type");
...
struct tag_count<Tag>
: std::integral_constant<std::size_t,0>
{
};

template<typename Tag,typename HeadTag,typename HeadOptionType,typename... Tail>
struct tag_count<Tag,tagged_argument<HeadTag,HeadOptionType>,Tail...>
: tag_count<Tag,Tail...>
{
};

template<typename Tag,typename HeadOptionType,typename... Tail>
struct tag_count<Tag,tagged_argument<Tag,HeadOptionType>,Tail...>
: std::integral_constant<std::size_t,tag_count<Tag,Tail...>::value + 1>
{
};
...
template<typename Tag,typename Options,typename DefaultValue,
         typename = typename std::enable_if<boost::fusion::result_of::has_key<Options,Tag>::type::value>::type>
inline auto get_option_value_or(const Options& options,const DefaultValue&)
     -> decltype(tagged_argument<Tag,DefaultValue>(boost::fusion::at_key<Tag>(options)))
{
    return tagged_argument<Tag,DefaultValue>(boost::fusion::at_key<Tag>(options));
}

template<typename Tag,typename Options,typename DefaultValue,
         typename = typename std::enable_if<!boost::fusion::result_of::has_key<Options,Tag>::type::value>::type>
inline auto get_option_value_or(const Options&,const DefaultValue& default_value)
...
{
public:
    option_parser(OptionType default_value_,ContinuationParser continuation_parser_)
    ...
    auto operator()(const Options& options)const
         -> decltype(boost::fusion::push_back(std::declval<ContinuationParser>()(options),
                                              get_option_value_or<Tag>(options,std::declval<OptionType>())))
    {
        return boost::fusion::push_back(continuation_parser_(options),
    ...
    {
        static_assert(tag_count<Tag,Head,Tail...>::value < 2,"option is ambiguous");
...
public:
    ...
    auto operator()(keyword<Tag,OptionValue>,const DefaultValue& default_value)const
         -> decltype(options_description<detail::option_parser<Tag,OptionValue,RootParser> >(
                       detail::option_parser<Tag,OptionValue,RootParser>(default_value,std::declval<RootParser>())))
    {
        return options_description<detail::option_parser<Tag,OptionValue,RootParser> >(
                 detail::option_parser<Tag,OptionValue,RootParser>(default_value,root_parser_));
    }

    template<typename... Options>
    auto parse(Options... options)
         -> decltype(boost::fusion::as_map(std::declval<RootParser>()(boost::fusion::map<Options...>(options...))))
    {
        root_parser_.verify(options...);

        auto parsed_options = boost::fusion::as_map(root_parser_(boost::fusion::map<Options...>(options...)));
...
inline auto get_option_value(keyword<Tag,OptionType>,const OptionMap& option_map)
...
#define ECHELON_DECLARE_KEYWORD(OPTION_TYPE,IDENTIFIER)      \
    ...
    {                                                        \
        struct IDENTIFIER {};                                \
                                                             ...
                                                             \
    constexpr ::echelon::utility::keyword<tag::IDENTIFIER,OPTION_TYPE> _##IDENTIFIER{};
