...
inline std::size_t map_indices(const std::vector<std::size_t>& shape,
                               IndexTypes... indices)
{
    assert(shape.size() == sizeof...(indices));

    const std::initializer_list<std::size_t> indices_ = {
        static_cast<std::size_t>(indices)...};

    auto first = boost::make_zip_iterator(
        boost::make_tuple(begin(shape), begin(indices_)));
    auto last =
        boost::make_zip_iterator(boost::make_tuple(end(shape), end(indices_)));

    return std::accumulate(
        first, last, static_cast<std::size_t>(0),
        [](std::size_t acc,
