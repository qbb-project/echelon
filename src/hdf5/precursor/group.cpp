...
{
    ECHELON_ASSERT_MSG(H5Iget_type(id()) == H5I_GROUP, "ID does not refer to a group");
...
hsize_t group::iterate_links(H5_index_t index_type, H5_iter_order_t order, hsize_t start_index,
                             std::function<herr_t(hid_t, const char*)> op) const
{
    ...
    herr_t error_code =
        H5Literate(id(), index_type, order, &current_index, &kubus_hdf5_iterate_proxy_op, &data);
                        ...
                        std::function<herr_t(hid_t, const char*)> visitor) const
{
    ...
    herr_t error_code = H5Lvisit(id(), index_type, order, &kubus_hdf5_iterate_proxy_op, &data);
                          ...
                          std::function<herr_t(hid_t, const char*)> visitor) const
{
    ...
    herr_t error_code =
        H5Ovisit(id(), index_type, order, &kubus_hdf5_visit_objects_proxy_op, &data);
