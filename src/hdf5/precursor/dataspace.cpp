...
namespace precursor
{
hyperslab_block::hyperslab_block(std::vector<hsize_t> start_, std::vector<hsize_t> opposite_)
: start_(start_), dims_(opposite_.size())
{
    std::transform(begin(opposite_), end(opposite_), begin(start_), begin(dims_),
                   std::minus<hsize_t>());
}
...
dataspace::dataspace(const std::vector<hsize_t>& dims, const std::vector<hsize_t>& max_dims)
...
void dataspace::select_hyperslab(H5S_seloper_t op, const std::vector<hsize_t>& start,
                                 const std::vector<hsize_t>& stride,
                                 const std::vector<hsize_t>& count,
                                 const std::vector<hsize_t>& block)
{
    herr_t error_code =
        H5Sselect_hyperslab(id(), op, start.data(), stride.data(), count.data(), block.data());
...
{
    herr_t error_code =
        H5Sselect_hyperslab(id(), op, start.data(), stride.data(), count.data(), nullptr);
...
void dataspace::select_hyperslab(H5S_seloper_t op, const std::vector<hsize_t>& start,
                                 const std::vector<hsize_t>& count)
{
    herr_t error_code = H5Sselect_hyperslab(id(), op, start.data(), nullptr, count.data(), nullptr);
