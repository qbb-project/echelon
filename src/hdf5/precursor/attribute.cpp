...
attribute::attribute(hid_t loc_id_, const std::string& name_, const type& attr_type_,
                     const property_list& acpl, const property_list& aapl)
: attribute_id_(
      H5Acreate2(loc_id_, name_.c_str(), attr_type_.id(), dataspace().id(), acpl.id(), aapl.id()))
{
    if (id() < 0)
        throw_on_hdf5_error();
}
